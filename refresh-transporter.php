...
if (isset($data['do_login'])){
    $errors = array();
    $transporter = R::findOne('transporter', 'transporter_phone = ?', array($data['phone']));
    if ($transporter){
        if ($transporter -> answer == $data['answer']){
            $transporter -> transporter_pass = password_hash($data['password2'], PASSWORD_DEFAULT);
            R::store($transporter);
            header('Location:login-transporter.php');
        } else{$errors[] = 'Не вірне слово';}
        ...
        <div class="row justify-content-center">
            <div class="col-md-5">
                <div class="login-bg login-transporter">
                    <form action="refresh-transporter.php" method="post" >
                        <div class="reg-input">
